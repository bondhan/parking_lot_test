...
func setStoreConnection(command *schema.Command) {
	...
	case string(schema.CMDRegistrationNumbersWithColor):
		command.Connection = Store.GetRegistrationsByColor()
	case string(schema.CMDSlotNumbersWithColor):
		command.Connection = Store.GetSlotsByColor()
	case string(schema.CMDSlotWithRegistration):
		command.Connection = Store.GetASlotByRegNo()
